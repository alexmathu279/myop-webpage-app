...
export default function SuspendForm({
  hospitalId,
  action,
}: {
  hospitalId: string
  ...
  return (
    <form action={action as any} style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid #e2e8f0', paddingTop: 16 }}>
      ...
      <textarea
        ...
        rows={3}
        required
        placeholder="Explain why this hospital is being suspended…"
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button type="submit" style={{ height: 36, padding: '0 16px', background: '#f59e0b', color: '#fff', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          Confirm suspension
        </button>
