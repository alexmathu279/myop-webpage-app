...
/**
 * app/(admin)/admin/hospitals/new/page.tsx
 * Admin direct hospital creation — multi-step, immediately approved.
 */

import { useActionState, useState, useRef } from 'react'
import { createHospital } from '@/lib/admin/hospitals'
...
export default function CreateHospitalPage() {
  const [state, formAction] = useActionState<ActionResult, FormData>(createHospital, initialState)
  ...
  const [selectedDepts, setSelectedDepts]         = useState<Set<string>>(new Set(HOSPITAL_DEPARTMENTS))
  ...
  function handleStep1() {
    ...
    if (module === 'hospital') setSelectedDepts(new Set(HOSPITAL_DEPARTMENTS))
    else setServices([])
    setStep(2)
  }
  ...
  return (
    <>
      <div className="page-header">
        <a href="/admin/hospitals" className="back-link">← Back to hospitals</a>
        <h1 className="page-title">Add new hospital</h1>
        <p className="page-sub">Created immediately as approved — visible to patients right away.</p>
      </div>
      ...
      <div className="create-layout">
        <div className="form-card">
          ...
          <form ref={formRef} action={formAction} noValidate>
            ...
            {module === 'hospital' && allDepts.filter(d => selectedDepts.has(d)).map(d =>
            ...
            {step === 1 && (
              <div className="step">
                <h2 className="step-title">Facility type</h2>
                <div className="module-options">
                  {MODULES.map(m => (
                    <label key={m.value} className={`module-option ${module === m.value ? 'is-selected' : ''}`}>
                      <input type="radio" checked={module === m.value} onChange={() => setModule(m.value as any)} className="sr-only" />
                ...
                <h2 className="step-title" style={{ marginTop: 16 }}>Basic information</h2>
                <div className="field-grid">
                  <div className="form-field col-full">
                    <label className="form-field__label">Facility name <span className="req">*</span></label>
                    <input type="text" className="form-field__input" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Apollo Hospitals" required maxLength={200} />
                  </div>
                  <div className="form-field">
                    <label className="form-field__label">Email <span className="req">*</span></label>
                    <input type="email" className="form-field__input" value={email} onChange={e => setEmail(e.target.value)} placeholder="info@hospital.com" required />
                  </div>
                  <div className="form-field">
                    <label className="form-field__label">Phone <span className="req">*</span></label>
                    <input type="tel" className="form-field__input" value={phone} onChange={e => setPhone(e.target.value)} placeholder="9876543210" required />
                  </div>
                  <div className="form-field">
                    <label className="form-field__label">Website <span className="opt">(optional)</span></label>
                    <input type="url" className="form-field__input" value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://hospital.com" />
                  </div>
                  <div className="form-field">
                    ...
                    <span className="form-field__hint">Platform commission per booking</span>
                  </div>
                </div>
                <button type="button" className="next-btn" onClick={handleStep1} disabled={!name.trim() || !email.trim() || !phone.trim()}>Continue →</button>
              </div>
            )}
              ...
              <div className="step">
                {module === 'hospital' && (
                  <>
                    ...
                    <p className="step-sub">All common departments are pre-selected. Uncheck any that don't apply.</p>
                ...
                {module === 'diagnostic' && (
                  <>
                    ...
                    <p className="step-sub">Select tests and services. Toggle 🏠 for home collection.</p>
              ...
              <div className="step">
                ...
                <div className="field-grid">
                  <div className="form-field col-full">
                    ...
                    <input type="text" className="form-field__input" value={address1} onChange={e => setAddress1(e.target.value)} placeholder="Building, street" required maxLength={200} />
                  </div>
                  <div className="form-field col-full">
                    <label className="form-field__label">Landmark / Area <span className="opt">(optional)</span></label>
                    <input type="text" className="form-field__input" value={address2} onChange={e => setAddress2(e.target.value)} placeholder="Near bus stand, MG Road" maxLength={200} />
                  </div>
                  <div className="form-field">
                    <label className="form-field__label">City <span className="req">*</span></label>
                    <input type="text" className="form-field__input" value={city} onChange={e => setCity(e.target.value)} placeholder="Kochi" required maxLength={100} />
                  </div>
                  <div className="form-field">
                  ...
                  <div className="form-field">
                    ...
                    <input type="text" className="form-field__input" value={pincode} onChange={e => setPincode(e.target.value)} placeholder="682001" required maxLength={6} />
                  </div>
                </div>

                <h2 className="step-title" style={{ marginTop: 8 }}>Description <span className="opt">(optional)</span></h2>
                <textarea className="form-field__textarea" value={description} onChange={e => setDescription(e.target.value)} placeholder="Brief description of the facility…" rows={3} maxLength={1000} />

                <div className="submit-summary">
                  <div className="summary-item"><span>Facility</span><strong>{name}</strong></div>
                  <div className="summary-item"><span>Type</span><strong>{MODULES.find(m => m.value === module)?.label}</strong></div>
                  <div className="summary-item"><span>Commission</span><strong>{commissionPct}%</strong></div>
                  ...
                  <div className="summary-item summary-item--approved"><span>Status</span><strong>✅ Immediately approved</strong></div>
                </div>

                <div className="step-nav">
                  <button type="button" className="back-btn" onClick={() => setStep(2)}>← Back</button>
                  <button type="submit" className="next-btn" disabled={!address1.trim() || !city.trim() || !stateVal || !pincode.trim()}>Create hospital →</button>
                </div>
              </div>
            )}
          </form>
        ...
        <aside className="side-note">
          <h3 className="side-note__title">ℹ Admin creation</h3>
          <p>Hospitals created here are <strong>immediately approved</strong> and visible to patients.</p>
          <p>For hospitals that apply themselves, direct them to <a href="/register">/register</a> — they go through the approval flow.</p>
        </aside>
      </div>
      ...
      <style>{`
        ...
        .create-layout { display: grid; grid-template-columns: 1fr 260px; gap: 24px; align-items: start; }
        ...
        .summary-item--approved strong { color: #166534; }
        ...
        .side-note { background: #f8fafc; border: 1px solid var(--myop-border); border-radius: var(--radius-md); padding: 20px; display: flex; flex-direction: column; gap: 10px; position: sticky; top: 80px; }
        .side-note__title { font-size: 13px; font-weight: 700; color: var(--myop-slate); margin-bottom: 2px; }
        .side-note p { font-size: 13px; color: var(--myop-muted); line-height: 1.5; }
        .side-note a { color: var(--myop-teal); text-decoration: none; }
        .side-note strong { color: var(--myop-slate); font-weight: 600; }
      `}</style>
    </>
  )
}